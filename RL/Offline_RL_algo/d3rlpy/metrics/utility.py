...
def evaluate_qlearning_with_environment_dict(
    algo: QLearningAlgoProtocol,
    env: GymEnv,
    n_trials: int = 10,
    epsilon: float = 0.0,
    ...
    for _ in range(n_trials):
        ...
        while True:
            ...
            if np.random.random() < epsilon:
                ...
                if isinstance(observation_array, np.ndarray):
                    observation_array = np.expand_dims(observation_array, axis=0)
                else:
                    raise ValueError(
                        f"Unsupported observation type: {type(observation_array)}"
                    )
                action = algo.predict(observation_array)[0]
