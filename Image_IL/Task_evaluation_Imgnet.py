...
resnet_model = load_Imgnet_pretrained_model()
resnet_model.eval()
...
def load_resnet_model(model_path, resnet_model):
    model = BehaviorCloningModel(resnet_model).to(device)
...
model_path = f'/home/jwu220/Trajectory_cloud/IL_model_v2/{task_name}/Imgnet_{view_name}_view/Model/model_final.pth'
model = load_resnet_model(model_path, resnet_model)
...
def run_experiment(num_episodes=20):
    ...
    for episode in range(num_episodes):
        ...
        for timestep in range(max_episode_steps):
            ...
            action[0:3] = action[0:3] + np.random.uniform(-0.5, 0.5, size=action[0:3].shape)
            if is_grasp:
                action[-1] = 0.0
...
results_dir = f"/home/jwu220/Trajectory_cloud/IL_model_v2/{task_name}/Imgnet_{view_name}_view/Results"
